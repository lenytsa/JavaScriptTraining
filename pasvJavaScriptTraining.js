...
console.log('-------------------------------------')
//Баллы за задачу.
// Сайт содержит набор задач по программированию.
//     Задачи имеют 3 уровня сложности: "easy", "medium", "hard".
//     За решение задачи уровня "easy" начисляется 2 балла, за решение задачи уровня "medium"
// начисляется 4 балла, за решение задачи уровня "hard" начисляется 8 баллов.
//
//     Напишите функцию scoreIncrease, которая принимает две величины current
// (текущее количество баллов) и level (уровень сложности задачи) в качестве
// аргументов и возвращает новое количество баллов, полученное
// после решения задачи. В решении необходимо использовать тернарный оператор.
// функция scoreIncrease(400, 'easy') должна возвратить 402;
// функция scoreIncrease(200, 'medium') должна возвратить 204;
// функция scoreIncrease(302, 'hard') должна возвратить 310.
function scoreIncrease(current, level){
return      level === 'easy'? current + 2:
            level === 'medium'? current + 4:
...
//Стоимость фруктов
// Торговец фруктов продает манго по 1 доллару за штуку. Но если покупатель берет 3 и больше манго,
//     то за один фрукт торговец просит 0.8 доллара. А если покупают
// 10 штук и более, то продавец готов продать их всего по 60 центов за штуку.
//
//     Напишите функцию saleFruits, которая принимает число n (количество фруктов) в
// качестве аргумента и возвращает стоимость покупки. Результат округлить до 2-х знаков после запятой.
//     В решении необходимо использовать тернарный оператор.
// функция saleFruits(2) должна возвратить "2.00";
// функция saleFruits(3) должна возвратить "2.40";
// функция saleFruits(10) должна возвратить "6.00.
...
if(n<3)
    ...
    return (n * 0.60).toFixed(2);
}
console.log(saleFruits(10));
function saleFruits1(n){
    return  n<3 ? (n * 1).toFixed(2):
            n >=3 && n<10 ? (n * 0.8).toFixed(2):
            n >= 10? (n * 0.60).toFixed(2): n;
}
