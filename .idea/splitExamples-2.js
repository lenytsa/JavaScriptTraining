...
Why does CodeWars not have the ability to DELETE old Katas?
    I cannot delete this, I cannot change the tests to improve it as they are locked for some reason. Instread people still try and complete this kata and rightfully add feedback that it's furstrating and makes little sense.

I can't understand why the functionality still has not been added to delete katas.

PLEASE DO NOT ATTEMPT TO DO THIS KATA*/
let Person = {
    city: 'Miami',
    state: 'FL',
    country: 'USA',
...
function bloodAlcoholContent({ounces, abv}, weight, sex, time){
    if(sex==='male')   return +((ounces * abv *5.14/weight* (0.73)) -.015*time).toFixed(4);
    if(sex === 'female') return +((ounces * abv *5.14/weight* (0.66)) - .015*time).toFixed(4);
}
...
function iTri(s){
    ...
    if (s<2.4) return {Swim:`${(tot-s).toFixed(2)} to go!`}
    if (s<2.4+112) return {Bike:`${(tot-s).toFixed(2)} to go!`}
    if (s<tot-10) return {Run:`${(tot-s).toFixed(2)} to go!`}
    if (s<tot) return {Run:'Nearly there!'}
