...
// Напишите функцию с именем calculate, которая принимает два числа
// a и b и строку operation (знак арифметической операции) в качестве аргументов, выполняет данную
// операцию над числами и возвращает результат в виде строки, содержащей решение. Возможные знаки
// операций: '+', '-', '*', '/', '%'. Результат деления чисел округлить до ближайшего целого числа с
// помощью метода Math.round(). Гарантируется, что во всех тестах будут использоваться только данные арифметические
// операции. В решении необходимо использовать оператор switch.
function calculate(a, b, operation){
    switch (operation) {
        case '+': return  a + ' + ' + b +' = '+ (a + b);
        case '-': return  a + ' - ' + b +' = '+ (a - b);
        case '*': return  a + ' * ' + b +' = '+ (a * b);
        case '/': return  a + ' / ' + b +' = '+ Math.round(a / b);
        case '%': return  a + ' % ' + b +' = '+ (a % b);
        default: 'Non a number';
    }
}
...
//Расстояние в метрах
// Дано целое число n (расстояние) и единица измерения этого расстояния ('km', 'm', 'dm', 'cm', 'mm').
// Получить расстояние в метрах.
//
//     Например, дано 5 km. Это соответствует расстоянию 5000 м.
//
//     Напишите функцию с именем unitsOfLength, которая принимает два аргумента: целое число n
//     (расстояние) и строку unit (единица измерения расстояния) и возвращает расстояние в метрах.
//     В решении необходимо использовать оператор switch.
//         функция unitsOfLength(5,'m') должна возвратить 5;
//     функция unitsOfLength(5,'km') должна возвратить 5000;
//     функция unitsOfLength(5,'dm') должна возвратить 0.5;
//     функция unitsOfLength(5, 'cm') должна возвратить 0.05;
//     функция unitsOfLength(5, 'mm') должна возвратить 0.005.
function unitsOfLength(n, unit){
    switch (unit) {
        case 'm': return n;
        case 'km': return n * 1000;
        case 'dm': return n * 0.1;
        case 'cm': return n * 0.01;
        case 'mm': return n * 0.001;
    }
}
console.log(unitsOfLength(5,'cm'));
console.log('---------------------------------------')


//Координаты Робота
// Известны начальные координаты Робота (x, y). Робот может двигаться в четырех направлениях:
// N - север, S - юг, W - запад, E - восток. При перемещении в определенном направлении из точки (x, y) соответствующая координата Робота изменяется на 1:
//
// при перемещении на север Робот оказывается в точке (x, y+1),
//     при перемещении на юг Робот оказывается в точке (x, y-1),
//     при перемещении на запад Робот оказывается в точке (x-1, y),
//     при перемещении на восток Робот оказывается в точке (x+1 y).
// Напишите функцию с именем robot, которая принимает три аргумента: числа x, y
// (начальные координаты Робота) и строку direction, содержащую направление движения
// ('N', 'S', 'E', 'W') и возвращает новые координаты Робота в виде массива x, y.
//
//     В решении необходимо использовать оператор switch.
//
//     Примеры:
//
//         функция robot(5, 5, 'E') должна возвратить [6, 5];
//     функция robot(5, 5, 'W') должна возвратить [4, 5];
//     функция robot(5, 5, 'N') должна возвратить [5, 6];
//     функция robot(5, 5, 'S') должна возвратить [5, 4].
